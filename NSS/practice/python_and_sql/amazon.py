...
class Amazon:
  ...
  def buy_amazon(self, quantity, purchase_price):
    ...
    with sqlite3.connect('example.db') as conn:
      ...
      #when inserting, the ?s are placeholders for the number of columns
  ...
  def query_all_amazon(self):
    ...
    with sqlite3.connect('example.db') as conn:
      ...
      print(c.fetchall())
      #all results from the select statement
  ...
  def query_latest_amazon(self):
    ...
    with sqlite3.connect('example.db') as conn:
      ...
      print(c.fetchone())
      #fetch only the first result instead of all of the results
...
#only commit after an action, not after a selection

#when you read from python you get a list of tuples
